...
export function createDurableExperimentStore(fileDir: string): ExperimentStore {
  ...
  return {
    ...
    async listPursuits(siteId, opts) {
      ...
      if (error) return fallback.listPursuits!(siteId, opts);
    ...
    async savePursuit(job: PursuitJob) {
      ...
      if (error) return fallback.savePursuit!(job);
    },
    ...
    async claimPursuits(input) {
      ...
      if (error) return fallback.claimPursuits!(input);
    ...
    async appendPursuitEvent(event: PursuitEvent) {
      ...
      if (error) return fallback.appendPursuitEvent!(event);
    },
    ...
    async listPursuitEvents(siteId, opts) {
      ...
      const { data, error } = await query;
      if (error) return fallback.listPursuitEvents!(siteId, opts);
      return (data ?? []).map(
        (row) =>
          ({
            id: row.id,
    ...
    async claimLease(input) {
      ...
      if (error) return fallback.claimLease!(input);
      return true;
    },
  };
}
