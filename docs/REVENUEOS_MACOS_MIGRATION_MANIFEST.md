...
| **WHERE MEMORY LIVES** | Supabase `revenueos_*` tables (active project `buvfllemxdvmwzhvfori` via root `.env.local`; ~47k experiments) + document jsonb; ephemeral `/tmp/revenueos` fallback on Vercel. Note: some `apps/*/.env.local` still point at stale/unreachable `fnrwzloovduhryynmgok`. | **Same active Supabase** (do not reset) + Mac local cache for speed |
