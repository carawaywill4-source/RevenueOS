...
/**
 * Commercially live when fulfillment path is ready.
 * Opt-out with NEXT_PUBLIC_CHECKOUT_ENABLED=0 — never require a separate opt-in
 * flag after Stripe + assets are present (that was a false LIVE blocker).
 */
export function digitalCommerciallyLive(gates: OwnerGate[]): boolean {
  if (process.env.NEXT_PUBLIC_CHECKOUT_ENABLED === "0") return false;
  return digitalCheckoutAllowed(gates);
}
