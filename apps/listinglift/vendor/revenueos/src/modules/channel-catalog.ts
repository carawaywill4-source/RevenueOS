...
export const CHANNEL_CAPABILITY_CATALOG: ChannelCapability[] = [
  ...
  },
  {
    ...
    category: "local_listings",
    label: "Google Business Profile free posts",
    defaultBuyerIntent: "high",
    mechanism: "owned_distribution",
    actionTypes: ["gbp_post", "gbp_qa_answer"],
    writeCredentialEnv: ["GOOGLE_SERVICE_ACCOUNT_JSON"],
    allowedActions: ["draft_post", "draft_qa_answer"],
    postingRules: [
      "Free GBP posts only — never buy ads",
      "Truthful offers",
      "Draft when SA lacks location Manager access",
    ],
    contentFormats: ["gbp_update", "gbp_offer", "qa_answer"],
  ...
  },
  {
    ...
    category: "local_listings",
    label: "Bing Places free business updates",
    defaultBuyerIntent: "medium",
    mechanism: "owned_distribution",
    actionTypes: ["bing_places_post"],
    writeCredentialEnv: ["BING_PLACES_TOKEN"],
    readCredentialEnv: ["BING_WEBMASTER_API_KEY"],
    allowedActions: ["draft_post"],
    postingRules: ["Free Places updates only", "No paid promotions"],
    contentFormats: ["places_update"],
    defaultCooldownMinutes: 720,
    effortEstimate: 1,
    neverBuy: true,
  },
  {
    ...
    category: "local_listings",
    label: "Apple Business Connect showcase",
    defaultBuyerIntent: "medium",
    mechanism: "owned_distribution",
    actionTypes: ["apple_business_showcase"],
    writeCredentialEnv: [
      "APPLE_BUSINESS_TEAM_ID",
      "APPLE_BUSINESS_KEY_ID",
      "APPLE_BUSINESS_PRIVATE_KEY",
    ],
    allowedActions: ["draft_showcase"],
    postingRules: ["Free Apple Maps listing only", "Accurate NAP"],
    contentFormats: ["showcase"],
    defaultCooldownMinutes: 1_440,
    effortEstimate: 2,
    neverBuy: true,
  },
  {
    ...
    platform: "nextdoor",
    category: "local_listings",
    label: "Nextdoor Business free neighborhood posts",
    defaultBuyerIntent: "high",
    mechanism: "owned_distribution",
    actionTypes: ["nextdoor_business_post"],
    writeCredentialEnv: ["NEXTDOOR_SESSION_COOKIE"],
    allowedActions: ["draft_post", "sidecar_post"],
    postingRules: [
      "Free Business Page posts only",
      "No Nextdoor Ads spend",
      "Local, helpful, non-spam",
    ],
    contentFormats: ["neighborhood_post"],
  ...
  },
  {
    ...
    platform: "yelp",
    category: "local_listings",
    label: "Yelp free owner posts + review responses",
    defaultBuyerIntent: "medium",
    mechanism: "owned_distribution",
    actionTypes: ["yelp_business_post", "yelp_review_response"],
    readCredentialEnv: ["YELP_API_KEY"],
    allowedActions: ["draft_post", "draft_review_response"],
    postingRules: ["Free Yelp for Business only", "No Yelp Ads", "Honest replies"],
    contentFormats: ["owner_post", "review_response"],
  ...
  },
  {
    ...
    platform: "youtube",
    category: "visual_discovery",
    label: "YouTube Shorts (free algorithmic distribution)",
    defaultBuyerIntent: "medium",
    mechanism: "owned_distribution",
    actionTypes: ["youtube_shorts_publish"],
    writeCredentialEnv: ["YOUTUBE_OAUTH_TOKEN"],
    readCredentialEnv: ["YOUTUBE_API_KEY"],
    allowedActions: ["draft_script", "optional_upload"],
    postingRules: ["No fake engagement", "No paid boosts", "One Short/day max"],
    contentFormats: ["shorts_script"],
    defaultCooldownMinutes: 1_440,
    effortEstimate: 3,
    neverBuy: true,
  },
